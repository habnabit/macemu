...
sheepshaver_state *the_app = NULL;
...
int main(int argc, char **argv)
{
	...
	for (int i=1; i<argc; i++) {
		...
		} else if (strcmp(argv[i], "--fast-playback") == 0) {
			argv[i] = NULL;
			the_app->fast_playback = true;
...
{
	the_app->init_emul_ppc();
	the_app->emul_ppc(entry);
}
#endif
